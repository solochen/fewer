package com.yilan.lib.playerlib.global;

/**
 * Created by chenshaolong on 2018/1/15.
 */

public class SPConstant {

    public static final String KEY_USER_INFO = "key_user_info";  //用户信息

    public static final String KEY_INVITE_CODE = "key_invite_code";  //邀请码

    public static final String KEY_REVIVE_COUNT = "key_revive_count";  //复活卡数量

//    public static final String KEY_IS_USE_REVIVE_CODE = "key_is_use_revive_code"; // 是否使用过复活卡

//    public static final String KEY_IS_TELL_USER_DIALOG = "key_is_tell_user";  //当前场 是否已经弹出了 "已迟到" 弹窗


    public static final String testUserInfoJson = "{\n" +
            "\t\"data\": {\n" +
            "\t\t\"user_id\": 5330362954,\n" +
            "\t\t\"name\": \"Nos21\",\n" +
            "\t\t\"screen_name\": \"Nos21\",\n" +
            "\t\t\"avatar_url\": \"http://wx.qlogo.cn/mmopen/Q3auHgzwzM7Uhu2Ockjyyq5PBAXqqO1olsUgymtwjhqsSPx0gY6kVibibcnqMicwhrpxX0gibSOV84UmDyCHKgaG5w/0\",\n" +
            "\t\t\"user_verified\": false,\n" +
            "\t\t\"verified_content\": \"\",\n" +
            "\t\t\"verified_agency\": \"\",\n" +
            "\t\t\"is_blocked\": 0,\n" +
            "\t\t\"is_blocking\": 0,\n" +
            "\t\t\"bg_img_url\": \"http://p3.pstatp.com/origin/60e0000116e1e21ff9a\",\n" +
            "\t\t\"gender\": 1,\n" +
            "\t\t\"media_id\": 0,\n" +
            "\t\t\"user_auth_info\": \"\",\n" +
            "\t\t\"industry\": \"\",\n" +
            "\t\t\"area\": \"\",\n" +
            "\t\t\"can_be_found_by_phone\": 1,\n" +
            "\t\t\"mobile\": \"\",\n" +
            "\t\t\"birthday\": \"\",\n" +
            "\t\t\"description\": \"\",\n" +
            "\t\t\"new_user\": 0,\n" +
            "\t\t\"session_key\": \"183545db9854f41f24cb80ace0a99c32\",\n" +
            "\t\t\"is_recommend_allowed\": 1,\n" +
            "\t\t\"recommend_hint_message\": \"�����舵�ㄨ��缁�������绮�涓�\",\n" +
            "\t\t\"connects\": [{\n" +
            "\t\t\t\"platform\": \"weixin\",\n" +
            "\t\t\t\"profile_image_url\": \"http://wx.qlogo.cn/mmopen/vi_32/PiajxSqBRaEKNt4AicfbqAEY0BrwRBOyUgRItkbDicAeicI9qzyKApk9h2ibrW8lRAfHLpDticjQnx8EwrPdVzYoUd0g/132\",\n" +
            "\t\t\t\"expired_time\": 1516000200,\n" +
            "\t\t\t\"expires_in\": 0,\n" +
            "\t\t\t\"platform_screen_name\": \"stsl\",\n" +
            "\t\t\t\"user_id\": 5330362952,\n" +
            "\t\t\t\"platform_uid\": \"ovToCjzT6d7_Lqnc_zrRH480Jspc\"\n" +
            "\t\t}],\n" +
            "\t\t\"followings_count\": 0,\n" +
            "\t\t\"followers_count\": 4,\n" +
            "\t\t\"visit_count_recent\": 0\n" +
            "\t},\n" +
            "\t\"message\": \"success\",\n" +
            "\t\"new_platform\": 0,\n" +
            "\t\"new_user\": 0,\n" +
            "\t\"session_key\": \"183545db9854f41f24cb80ace0a99c32\",\n" +
            "\t\"should_bind_mobile\": 0\n" +
            "}";


}
